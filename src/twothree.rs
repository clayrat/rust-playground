...
impl<T: Ord> TwoThreeTree<T> {
    ...
    fn node_two(x: T, left: Box<Self>, right: Box<Self>) -> Self {
        TwoThreeTree::Two(left.size() + right.size() + 1, x, left, right)
    }
    fn node_three(x: T, y: T, left: Box<Self>, middle: Box<Self>, right: Box<Self>) -> Self {
        TwoThreeTree::Three(
            left.size() + middle.size() + right.size() + 2,
            x,
            y,
            left,
            middle,
    ...
    fn insert_sub(self, x: T) -> Result<Self, (Self, Self, T)> {
        ...
        match self {
            Tip => Err((Tip, Tip, x)),
            Two(size, val, left, right) => match x.cmp(&val) {
                std::cmp::Ordering::Equal => Ok(Two(size, val, left, right)),
                std::cmp::Ordering::Less => match left.insert_sub(x) {
                    Ok(t) => Ok(Self::node_two(val, Box::new(t), right)),
                    Err((t1, t2, sub_up)) => Ok(Self::node_three(
                        sub_up,
                        val,
                        Box::new(t1),
                        Box::new(t2),
                ...
                std::cmp::Ordering::Greater => match right.insert_sub(x) {
                    Ok(t) => Ok(Self::node_two(val, left, Box::new(t))),
                    Err((t1, t2, sub_up)) => Ok(Self::node_three(
                        val,
                        sub_up,
                        left,
            ...
            Three(size, val1, val2, left, middle, right) => {
                ...
                if x < val1 {
                    ...
                    match middle.insert_sub(x) {
                        Ok(sub_tr) => {
                            Ok(Self::node_three(val1, val2, left, Box::new(sub_tr), right))
                        }
                        Err((t1, t2, sub_up)) => {
                            let (t1, t2, v) = Self::divide_four(
                                left,
                    ...
                    match right.insert_sub(x) {
                        Ok(sub_tr) => {
                            Ok(Self::node_three(val1, val2, left, middle, Box::new(sub_tr)))
                        }
                        Err((t1, t2, sub_up)) => {
                            let (t1, t2, v) = Self::divide_four(
                                left,
    ...
    pub fn insert(self, x: T) -> Self {
        match self.insert_sub(x) {
            Ok(t) => t,
            Err((t1, t2, v)) => Self::node_two(v, Box::new(t1), Box::new(t2)),
        }
    }
    ...
    fn delete_sub(self, x: &T, mut leftist: bool) -> (Result<Self, Box<Self>>, Option<T>) {
        use TwoThreeTree::*;
        match self {
            Tip => (Ok(Tip), None),
                ...
                match cmp_result {
                    ...
                    std::cmp::Ordering::Less => {
                        match left.delete_sub(x, leftist) {
                            (Ok(t), ret) => (Ok(Self::node_two(val, Box::new(t), right)), ret),
                            (Err(box_t), ret) => {
                                match *right {
                                    Tip => panic!(),
                    ...
                    std::cmp::Ordering::Greater => {
                        match right.delete_sub(x, leftist) {
                            (Ok(t), ret) => (Ok(Self::node_two(val, left, Box::new(t))), ret),
                            (Err(box_t), ret) => {
                                match *left {
                                    Tip => panic!(),
            ...
            Three(_, mut val1, mut val2, left, mut middle, mut right) => {
                ...
                if cmp_result == 1 {
                    if let Tip = *right {
                        return (
                            Ok(Self::node_two(val2, Box::new(Tip), Box::new(Tip))),
                            Some(val1),
                        );
                    }
                ...
                if cmp_result == 3 {
                    if let Tip = *right {
                        return (
                            Ok(Self::node_two(val1, Box::new(Tip), Box::new(Tip))),
                            Some(val2),
                        );
                    }
                    let inord_succ = right.get_min();
                ...
                match cmp_result {
                    0 => match left.delete_sub(x, leftist) {
                        (Ok(t), ret) => (
                            Ok(Self::node_three(val1, val2, Box::new(t), middle, right)),
                            ret,
                        ),
                        (Err(t), ret) => match (*middle, *right) {
                            (Three(_, mv1, mv2, mleft, mmiddle, mright), right) => (
                                Ok(Self::node_three(
                                    mv1,
                                    val2,
                            ...
                            (
                                ...
                                Ok(three_two!(
                                    t, val1, mleft, mv, mright, val2, rleft, rv1, rmiddle, rv2,
                                    rright
                                )),
                                ret,
                            ),
                            (Two(_, mv, mleft, mright), right) => (
                                Ok(Self::node_two(
                                    val2,
                                    Box::new(Self::node_three(val1, mv, t, mleft, mright)),
                                    Box::new(right),
                                )),
                                ret,
                            ),
                    ...
                    2 => match middle.delete_sub(x, leftist) {
                        (Ok(t), ret) => (
                            Ok(Self::node_three(val1, val2, left, Box::new(t), right)),
                            ret,
                        ),
                        (Err(t), ret) => match (*left, *right) {
                            (Three(_, lv1, lv2, lleft, lmiddle, lright), right) => (
                                Ok(Self::node_three(
                                    lv2,
                                    val2,
                            ...
                            (
                                ...
                                Ok(three_two!(
                                    lleft, lv, lright, val1, t, val2, rleft, rv1, rmiddle, rv2,
                                    rright
                                )),
                                ret,
                            ),
                            (Two(_, lv, lleft, lright), right) => (
                                Ok(Self::node_two(
                                    val2,
                                    Box::new(Self::node_three(lv, val1, lleft, lright, t)),
                                    Box::new(right),
                                )),
                                ret,
                            ),
                    ...
                    4 => match right.delete_sub(x, leftist) {
                        (Ok(t), ret) => (
                            Ok(Self::node_three(val1, val2, left, middle, Box::new(t))),
                            ret,
                        ),
                        (Err(t), ret) => match (*left, *middle) {
                            (left, Three(_, mv1, mv2, mleft, mmiddle, mright)) => (
                                Ok(Self::node_three(
                                    val1,
                                    mv2,
                                    Box::new(left),
                                    Box::new(Self::node_two(mv1, mleft, mmiddle)),
                                    Box::new(Self::node_two(val2, mright, t)),
                                )),
                                ret,
                            ),
                            (
                                ...
                                Ok(three_two!(
                                    lleft, lv1, lmiddle, lv2, lright, val1, mleft, mv, mright,
                                    val2, t
                                )),
                                ret,
                            ),
                            (left, Two(_, mv, mleft, mright)) => (
                                Ok(Self::node_two(
                                    val1,
                                    Box::new(left),
                                    Box::new(Self::node_three(mv, val2, mleft, mright, t)),
                                )),
                                ret,
                            ),
                            _ => panic!("Unexpected Tip"),
    ...
    pub fn find_index(&self, x: &T) -> (usize, bool) {
        ...
        match *self {
            Tip => (0, false),
            Two(_size, ref v, ref left, ref right) => match x.cmp(v) {
